...
//それぞれのusersのcurrently_datingを0、is_privateを1に変更、datesのis_datingを0に変更
$updateDatingStmt = $conn->prepare("UPDATE users SET currently_dating = 0, is_private = 1 WHERE user_id = :sender_id");
$updateDatingStmt->execute([':sender_id' => $sender_id]);
$updateDatingStmt = $conn->prepare("UPDATE users SET currently_dating = 0, is_private = 1 WHERE user_id = :receiver_id");
...
header("Location: date_success.php");
exit();
